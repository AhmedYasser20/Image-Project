...
def getNoteCharacter(originalImage, boundingRect, noteClass, lines, staffHeight, spaceHeight):
    ...
    try:
        if noteClass == 'a_4' or noteClass == 'a_8' or noteClass == 'a_16' or noteClass == 'a_32':
            # Preprocess the contour image
            yprojection = np.sum(contourImage//255, axis=0)
            yprojection = np.where(yprojection > spaceHeight + 2*staffHeight)
            contourImage[:,yprojection] = 0
            ...
            for i in range(w):
                ...
                if len(starts_ix) > 0 and len(ends_ix) > 0:
                    index = np.argmax(ends_ix - starts_ix)
                    hist[i,1] = i
                    hist[i,2] = starts_ix[index]
                    hist[i,3] = ends_ix[index]
            ...
            if len(peaks) == 0:
                ...
                _, top, distanceTop = getNearestLine(center_y - spaceHeight//2, lines)
                _, bottom, distanceBottom = getNearestLine(center_y + spaceHeight//2, lines)
                character = getHeadCharacter(top, distanceTop, bottom, distanceBottom, spaceHeight)
            else:
                widths = peak_widths(hist[:,0], peaks)[0]
                if len(widths) > 0:
                    ...
                    raise ValueError("No valid note heads detected")
            
            # Add duration to character
            if noteClass == 'a_4':
                character += '/4'
            elif noteClass == 'a_8':
                character += '/8'
            ...
            # Handle other note types (a_1, a_2, beam) as before
            # [Previous implementation...]
            pass
            
    except Exception as e:
        # Fallback behavior for any errors
        center_y = (min_y + max_y) // 2
        _, top, distanceTop = getNearestLine(center_y - spaceHeight//2, lines)
        _, bottom, distanceBottom = getNearestLine(center_y + spaceHeight//2, lines)
        character = getHeadCharacter(top, distanceTop, bottom, distanceBottom, spaceHeight)
        character += '/4'  # Default to quarter note if detection fails
